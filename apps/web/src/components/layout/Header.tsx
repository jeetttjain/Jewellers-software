...
export const Header: React.FC<HeaderProps> = ({ onToggleSidebar }) => {
  return (
    <header className="flex-shrink-0 z-30 bg-white border-b border-surface-200 h-14 px-2 sm:px-4 w-full max-w-full overflow-hidden flex items-center justify-between shadow-xs">
      {/* Left: Mobile menu toggle + Store Brand */}
      <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
        {onToggleSidebar && (
          <button
            onClick={onToggleSidebar}
            className="p-1.5 rounded-md text-surface-700 hover:bg-surface-100 lg:hidden focus:outline-none cursor-pointer"
        ...
        <Link to="/dashboard" className="flex items-center gap-1.5 sm:gap-2">
          ...
          <span className="font-bold text-xs sm:text-sm tracking-tight text-surface-900 hidden xs:inline sm:inline">
            Kamal Jewellers
          </span>
        </Link>
      ...
      <div className="flex items-center gap-1.5 sm:gap-4 overflow-x-auto py-1 max-w-[140px] xs:max-w-none sm:max-w-none no-scrollbar">
        <div className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 bg-gold-50 border border-gold-200 rounded-md text-[11px] sm:text-xs font-mono font-bold text-gold-900 shadow-2xs whitespace-nowrap">
          <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gold-600 animate-pulse flex-shrink-0" />
          <span className="text-[10px] sm:text-[11px] text-gold-700 font-sans font-medium hidden xxs:inline">22K Gold:</span>
          <span>₹6,450/g</span>
        </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-1 px-1.5 sm:px-2 py-1 text-[10px] sm:text-[11px] font-medium text-emerald-700 bg-emerald-50 rounded-md border border-emerald-200">
